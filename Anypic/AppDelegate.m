...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [Parse setApplicationId:@"1PjDJ9cW3vgdJtIXhWSe6OX1rXEEGjVlT8WYd5Sl"
                  clientKey:@"t0RJX2JQr52sKgpbMyUyPVquGnzAi0WXrJBu0djS"];
...
- (BOOL)shouldProceedToMainInterface:(PFUser *)user {
    if ([PAPUtility userHasValidFacebookData:[PFUser currentUser]]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Terms" message:@"You must use your real name. Please, no hazing or other unwanted horseplay. There is zero tolerance for objectionable content! If you abuse this platform or otherwise are a nuisance we reserve the right to suspend your account. Keep it classy. If you find someone is creating objectionable content or otherwise creating an unfriendly environment, please flag it and it will be deleted. Thanks and have fun!" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Ok",nil];
        [alert show];
    ...
    if (data) {
        ...
        if (user) {
            if (![user objectForKey:kPAPUserAlreadyAutoFollowedFacebookFriendsKey]) {
                ...
                // find common Facebook friends already using Azadi
